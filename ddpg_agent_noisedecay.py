...
class DDPG():
    ...
    def act(self, state, i_episode, add_noise=True):
        ...
        if add_noise:
            self.noise_scale = self.noise_scale*NOISE_DECAY if i_episode>REPLAY_START_EPISODE else 1           
            action += min(self.noise_scale, NOISE_MIN)*0.5*np.random.randn(1,self.action_size)
